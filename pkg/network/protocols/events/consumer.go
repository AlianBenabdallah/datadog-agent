...
// `callback` is executed once for every "event" generated on eBPF and it's the
// caller responsibility to convert the slice of bytes into whatever type is
// used in eBPF *and* copy its contents where applicable, since the memory of
// the byte slice is reclaimed after the execution of the callback
